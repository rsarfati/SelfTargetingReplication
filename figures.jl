...
function figure_6(; labels::Dict{String,String} = labels)
 ...
 # Extension
 ###########################
 function extension(; labels::Dict{String,String} = labels,
                     N_bs = 1000, α = 0.05, run_bootstrap = true)
     # Load and clean data
     df = rename!(DataFrame(load("input/matched_baseline.dta")),
                  [:logconsumption  => :logc])
    ...
    df = innerjoin(df, df_show[:, [to_keep..., :hhid]], on = :hhid)
    df = clean(df, [:distt, :showup], F64)

     # Define grid of points at which to evaluate regression
     x0_grid = collect(range(minimum(df.distt); stop = maximum(df.distt), length = 200))

     # Run Fan regression on aforementioned grid
     y_hat, ub, lb = fan_reg(@formula(showup ~ distt), df, x0_grid; clust = :hhea,
                             bw = :cross_val, α = α, run_bootstrap = run_bootstrap,
                             N_bs = N_bs, caller_id = "Extension")
     # Plot and save
     p = plot(x0_grid, y_hat, legend = false,
              xl = "Distance to Test Site", yl = "Show-up Probability", lw=2., lc=:cyan3)
     plot!(p, x0_grid, [lb, ub], ls = :dash, lw = 1.5, lc = :cyan4)
     savefig(p, "output/plots/extension1.png")
     println("(Extension - Plot 1) ✅")

     df1 = df[df.verypoor_povertyline2 .== 0, :]
     df2 = df[df.verypoor_povertyline2 .== 1, :]

     # Run Fan regression on aforementioned grid
     y_hat, ub, lb = fan_reg(@formula(showup ~ distt), df1, x0_grid; clust = :hhea,
                             bw = :cross_val, α = α, run_bootstrap = run_bootstrap,
                             N_bs = N_bs, caller_id = "Extension")
     # Plot and save
     p = plot(x0_grid, y_hat, legend = false,
              xl = "Distance to Test Site", yl = "Show-up Probability", lw=2., lc=:cyan3)
     plot!(p, x0_grid, [lb, ub], ls = :dash, lw = 1.5, lc = :cyan4)
     savefig(p, "output/plots/extension2.png")
     println("(Extension - Plot 2) ✅")

     # Run Fan regression on aforementioned grid
     y_hat, ub, lb = fan_reg(@formula(showup ~ distt), df2, x0_grid; clust = :hhea,
                             bw = :cross_val, α = α, run_bootstrap = run_bootstrap,
                             N_bs = N_bs, caller_id = "Extension")
     # Plot and save
     p = plot(x0_grid, y_hat, legend = false,
              xl = "Distance to Test Site", yl = "Show-up Probability", lw=2., lc=:cyan3)
     plot!(p, x0_grid, [lb, ub], ls = :dash, lw = 1.5, lc = :cyan4)
     savefig(p, "output/plots/extension3.png")
     println("(Extension - Plot 3) ✅")
     return p
 end
